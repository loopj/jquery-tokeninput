...
*
* Mods by Sunny Walker, http://hilo.hawaii.edu/
...
(function($) {
	...
	$.TokenList = function (input, settings) {
		...
		// Save the tokens for duplicate checking
		var saved_tokens = [];
		...
		function add_token (item) {
			...
			if (token_count > 0 && settings.preventDuplicates) {
				...
				if (idx>-1) {
					select_token(saved_tokens[idx]); // Select the existing token
					input_token.insertAfter(saved_tokens[idx]); // Move the input area to the correct place
					input_box.focus();
					return; // Don't insert the token, because it already exists
				}
			}
		...
		function find_saved_token (id) {
			...
			for (var i=0; i<saved_tokens.length; i++) {
				if ($.data($(saved_tokens[i]).get(0), "tokeninput").id == id) {
					idx = i;
					break; // We found it, no need to continue searching
				}
			}
			return idx;
		} //find_saved_token ()
